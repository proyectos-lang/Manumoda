...
--   3. Alta de `sin_contramuestra` ($500, manual).
--   4. Alta de `parcialidad_excedente` ($500, inactiva): es el
--      parámetro del monto por excedente, no un check. Inactiva
--      para que no aparezca como casilla en la pantalla del folio.
--
--   La muestra original y la contramuestra son dos entregas
--   distintas del proceso de diseño, así que van como conceptos
--   separados y se pueden cobrar de forma independiente.
--
...
-- 2. Conceptos manuales nuevos.
INSERT INTO manumoda.cat_penalizaciones_maquila (idempresa, clave, nombre, monto, orden)
VALUES
  (1, 'sin_muestra_original', 'Sin entrega de muestra original', 500.00, 40),
  (1, 'sin_contramuestra',    'Sin entrega de contramuestra',    500.00, 50)
...
--      activa    Sin entrega de contramuestra         500
SELECT clave, nombre, monto, orden, activo
FROM manumoda.cat_penalizaciones_maquila
WHERE idempresa = 1
ORDER BY orden;

-- 2. Los conceptos que se ofrecen como check deben ser exactamente cuatro:
--    Packing List, no apartó fecha, muestra original y contramuestra.
